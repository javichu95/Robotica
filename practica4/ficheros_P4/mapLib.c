...
void detectObstacle(){
	...
	int distancia = 20;		// Distancia al obstáculo.

	if(SensorValue[sensorSONAR] <= distancia){
		// Se ha detectado el obstáculo.
		float x,y, theta;			// Variables para la odometría.
		readOdometry(x,y,theta);		// Se lee la odometría.

		// Se obtiene la celda en la que está el obstáculo.
		int celdaX = (int)(x/sizeCell) + pathX[0];
		int celdaY = (int)(y/sizeCell) + pathY[0];
...
* dada.
*/
void go(int cellX, int cellY, bool mismaOri){

	float x, y, theta;			// Variables para la odometría.
	readOdometry(x,y,theta);			// Se lee la odometría.
	// Se saca la celda en la que estamos.
	int coordX = (redondearCoord(x)/sizeCell)+pathX[0];
	int coordY = (redondearCoord(y)/sizeCell)+pathY[0];
	...
	//detectObstacle();			// Se detecta si hay un obstáculo.
...
void recorrerCamino() {
	...
	while(grid[pathX[ind]][pathY[ind]] != 0){			// Mientras no se llegue al objetivo.
		...
		if(ori != 'X' && pathX[ind-1] == pathX[ind]){
			...
			go(pathX[ind],pathY[ind], mismaOri);		// Indica que comience a avanzar.
